...
export const LoadData = ({ languageFilter }) => {
  ...
  const loadDataFromFB = async () => {
    ...
    if (languageFilter) {
      ...
      queryRef = query(ref(db, 'teachers'), orderByKey(), startAt(String(currentOffset)), endBefore(String(currentOffset + limit)), orderByChild('language').equalTo(languageFilter));
    } else {
